...
本文是当前完整版本全部 HTTP 接口的公共契约。[基础交易接口分册](phase-1-api.md)和[治理与售后接口分册](phase-2-api.md)按业务主题拆分，二者并集组成当前接口范围；未重复说明的内容一律继承本文。后续规划中的三期接口不属于当前完整版本，只有在三期冻结时同步更新本文和 DTO 字段目录后，才成为正式契约。
...
| 413 | `PAYLOAD_TOO_LARGE` | 请求体或上传文件超过服务端限制；图片上传统一使用 `UPLOAD_FILE_TOO_LARGE` |
| 422 | `BUSINESS_RULE_VIOLATION` | 请求格式正确但违反业务规则 |
| 422 | `STORAGE_NOT_CONFIGURED` | 三期图片上传未启用 MinIO |
...
平台将店铺改为 `CLOSED` 前，必须确认没有 `PENDING_PAYMENT`、`PENDING_SHIPMENT`、`PENDING_RECEIPT` 子订单、`PENDING`/`WAITING_RETURN`/`REFUNDING` 售后和 `after_sale_appeal.status=PENDING` 的申诉；否则返回 `SHOP_HAS_ACTIVE_BUSINESS`。暂停店铺不取消已有订单。
...
以下操作必须提交 `Idempotency-Key`：创建交易、创建/执行支付、模拟充值、库存入库/调整、创建售后、提交退货物流、售后批准、确认退货收货并退款、执行或重试退款、提交售后申诉、平台裁决售后申诉。售后批准接口包含可能立即退款的仅退款分支，因此整个接口统一必填，不能由客户端按售后类型决定是否提交；平台裁决同样可能立即触发退款，必须使用幂等键。
...
适用资源：SKU 价格/启停、二期库存人工调整、售后审核动作中的版本校验。库存入库、订单、交易和支付动作虽然内部使用版本或行锁，但客户端不提交版本，后端以状态条件和幂等键处理；库存人工调整是例外，必须提交请求体中的 `version`。未来三期商家钱包人工调账若纳入正式契约，也必须沿用同样的版本规则。

SPU 使用 `contentVersion` 表示受审核内容版本。内容更新请求必须提交当前 `contentVersion`，冲突同样返回 `VERSION_CONFLICT`。

### 9.2 状态动作

审核、上架、支付、取消、发货、收货、售后审核、售后申诉裁决和退款必须使用专门动作接口，禁止前端直接 PATCH 状态字段。
