...
| 平台店铺管理员 `PLATFORM_SHOP_ADMIN` | 管理入驻店铺状态并处理平台售后申诉 | 创建店铺、编辑店铺、启用、暂停、关闭；查看申诉证据并作出平台裁决 |
...
| 售后退款与申诉 | `after_sale_request`；申诉和商家通知由 `scheme3.sql` 增量提供 | 申请、撤销、商家审核、平台申诉裁决、退货信息、收货、退款、失败重试和商家通知 | 治理与售后 |
...
6. 商家拒绝或 48 个自然小时未处理时，买家可以提交一次申诉；平台管理员查看双方证据并作出批准或驳回裁决。
7. 平台批准申诉时沿用原售后类型：仅退款直接进入退款，退货退款进入待退货；裁决结果和原因通知对应店铺的有效售后成员。
8. 钱包退款、钱包流水、订单累计退款、明细累计退款和售后资金状态在同一事务内更新；退货入库以独立业务唯一键防止退款重试时重复回库。
...
| 图片 | 三期统一通过 MinIO 上传 JPEG、PNG、GIF、WEBP 图片；业务接口继续接收资源 URL，并兼容旧的 HTTPS URL |
...
| 商家结算、佣金、提现 | 三期已单独设计店铺虚拟钱包和结算迁移；真实银行/第三方出款仍不做 |
| 评价、收藏、足迹、推荐 | 无对应表和搜索/推荐基础设施 |
| 多仓、运费模板、拆包裹 | 当前一 SKU 一库存、一子订单一组物流字段 |
| 换货、维修 | 当前售后状态只覆盖仅退款和退货退款；平台申诉裁决仅处理现有两类售后，不扩展换货或维修 |
| 店铺装修、营销中心、客服聊天 | 超出当前教学交易闭环 |
| 泛化文件存储、视频、压缩包和任意附件 | 三期只提供图片对象存储，不开放通用文件平台 |
