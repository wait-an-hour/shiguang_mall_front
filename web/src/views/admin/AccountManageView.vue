...
async function loadData() {
  ...
  try {
    ...
    rows.value = data.items.map((item) => ({
      ...
      role: item.platformRoles[0]?.roleCode === 'SUPER_ADMIN' ? 'SUPER_ADMIN' : item.platformRoles[0]?.roleCode === 'AUDIT_ADMIN' ? 'AUDIT_ADMIN' : 'OPERATION_ADMIN',
      permissions: [],
      status: item.status,
      ...
      <el-form :model="form" label-width="90px">
        ...
        <el-form-item label="角色">
          <el-select v-model="form.role">
            ...
            <el-option label="售后审核员" value="AUDIT_ADMIN" />
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
