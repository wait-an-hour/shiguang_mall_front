...
async function freeze(row: PlatformAccount) { await setAccountStatus(row.id, row.status === 'ACTIVE' ? 'DISABLED' : 'ACTIVE'); ElMessage.success('账号状态已更新'); loadData() }
...
<template>
  <div class="page-view">
    ...
    <SearchPanel><el-form><el-form-item label="关键词"><el-input v-model="query.keyword" placeholder="账号/姓名/店铺" clearable /></el-form-item><el-form-item label="状态"><el-select v-model="query.status" clearable><el-option label="正常" value="ACTIVE" /><el-option label="停用" value="DISABLED" /><el-option label="锁定" value="LOCKED" /></el-select></el-form-item><el-button type="primary" @click="loadData">查询</el-button></el-form></SearchPanel>
    <el-card class="sg-card" shadow="never" v-loading="loading"><el-table v-if="rows.length" :data="rows"><el-table-column prop="username" label="账号" /><el-table-column prop="displayName" label="名称" /><el-table-column prop="role" label="角色" /><el-table-column prop="ownerShopName" label="关联店铺" /><el-table-column label="状态"><template #default="{ row }"><StatusTag :label="row.status === 'ACTIVE' ? '正常' : row.status === 'LOCKED' ? '锁定' : '停用'" :type="row.status === 'ACTIVE' ? 'success' : 'danger'" /></template></el-table-column><el-table-column label="操作" width="260"><template #default="{ row }"><div class="table-actions"><el-button link type="primary" @click="openEdit(row)">编辑</el-button><ConfirmActionButton text="重置密码" confirm-text="确认重置该账号密码？" @confirm="resetPwd" /><ConfirmActionButton :text="row.status === 'ACTIVE' ? '停用' : '启用'" type="danger" confirm-text="确认变更该账号状态？" @confirm="freeze(row)" /></div></template></el-table-column></el-table><EmptyState v-else description="暂无账号，请调整筛选条件" /><AppPagination :page="query.page!" :page-size="query.pageSize!" :total="total" @change="Object.assign(query, $event); loadData()" /></el-card>
