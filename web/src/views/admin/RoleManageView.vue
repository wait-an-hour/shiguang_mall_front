...
import { deleteRole, listRoles, saveRole } from '@/api/admin/rbac'
...
<template>
  <div class="page-view">
    ...
    <el-dialog v-model="dialogVisible" title="角色权限" width="560px"><el-form :model="form" label-width="90px"><el-form-item label="名称"><el-input v-model="form.name" /></el-form-item><el-form-item label="编码"><el-select v-model="form.code"><el-option label="超级管理员" value="SUPER_ADMIN" /><el-option label="运营管理员" value="OPERATION_ADMIN" /><el-option label="售后审核员" value="AUDIT_ADMIN" /></el-select></el-form-item><el-form-item label="说明"><el-input v-model="form.description" /></el-form-item><el-form-item label="权限"><el-tree :data="permissionTree" show-checkbox node-key="id" :default-checked-keys="form.permissions" @check="(_node: unknown, data: PermissionTreeCheckInfo) => form.permissions = data.checkedKeys.filter((item) => item.startsWith('admin:')) as PermissionCode[]" /></el-form-item></el-form><template #footer><el-button @click="dialogVisible=false">取消</el-button><el-button type="primary" @click="submit">保存</el-button></template></el-dialog>
  </div>
</template>
