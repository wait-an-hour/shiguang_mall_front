...
export const mockAdmin = {
  ...
  listAccounts: (q: ListQuery) => delay(page(accounts.filter((i) => keywordMatch([i.username, i.displayName, i.ownerShopName ?? ''], q.keyword) && (!q.status || i.status === q.status)), q)), saveAccount: (record: PlatformAccount) => { accounts = record.id ? accounts.map((item) => item.id === record.id ? record : item) : [{ ...record, id: `acc-${Date.now()}`, createdAt: now }, ...accounts]; return delay(true) }, setAccountStatus: (id: Id, status: PlatformAccount['status']) => { accounts = accounts.map((item) => item.id === id ? { ...item, status } : item); return delay(true) },
