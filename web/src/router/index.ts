...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    ...
    children: [
      ...
      {
        path: 'wallet',
        name: ROUTE_NAME.MerchantWallet,
        component: () => import('@/views/merchant/MerchantWalletView.vue'),
        meta: {
          title: '商家钱包', layout: 'merchant', requiresAuth: true, shopScoped: true,
          permissions: [SHOP_PERMISSION.WalletRead]
        }
      },
      {
        path: 'coupons',
        name: ROUTE_NAME.MerchantCoupons,
        component: () => import('@/views/merchant/coupon/MerchantCouponCenterView.vue'),
        meta: {
          title: '优惠券管理', layout: 'merchant', requiresAuth: true, shopScoped: true,
          permissions: [SHOP_PERMISSION.CouponRead, SHOP_PERMISSION.CouponGrant, SHOP_PERMISSION.CouponFundingApprove],
          permissionMode: 'any'
        }
      },
      {
  ...
  },
  {
    ...
    children: [
      ...
      { path: 'shops/create', name: ROUTE_NAME.AdminShopCreate, component: () => import('@/views/admin/ShopCreateView.vue'), meta: { ...adminRouteMeta, title: '创建店铺', permissions: ['admin:shop:manage'] } },
      ...
      { path: 'shops/:shopId/edit', name: ROUTE_NAME.AdminShopEdit, component: () => import('@/views/admin/ShopEditView.vue'), meta: { ...adminRouteMeta, title: '编辑店铺', permissions: ['admin:shop:manage'] } },
      { path: 'shops/:shopId', name: ROUTE_NAME.AdminShopDetail, component: () => import('@/views/admin/ShopDetailView.vue'), meta: { ...adminRouteMeta, title: '店铺详情', permissions: ['admin:shop:manage'] } },
      ...
      { path: 'coupons', name: ROUTE_NAME.AdminCoupons, component: () => import('@/views/admin/CouponManageView.vue'), meta: { ...adminRouteMeta, title: '优惠券管理', permissions: ['platform:coupon:read'] } }
    ]
  },
  {
    path: '/403',
...
router.beforeEach((to) => {
  ...
  if (to.meta.shopScoped) {
    ...
    if (permissions.length > 0) {
      const hasPermission = to.meta.permissionMode === 'any'
        ? merchantStore.hasAnyShopPermission(permissions)
        : merchantStore.hasEveryShopPermission(permissions)
      if (!hasPermission) {
        return { name: ROUTE_NAME.Forbidden }
      }
    }
